import account from"../scripts/accounts.js";import questionStats from"../scripts/auth/question-stats.js";import api from"../scripts/api/index.js";import audio from"../audio/index.js";import Timer from"../scripts/Timer.js";import{arrayToRange,createTossupCard,rangeToArray}from"../scripts/utilities/index.js";import CategoryManager from"../scripts/utilities/category-manager.js";import{getDropdownValues}from"../scripts/utilities/dropdown-checklist.js";import{insertTokensIntoHTML}from"../scripts/utilities/insert-tokens-into-html.js";import CategoryModal from"../scripts/components/CategoryModal.min.js";import DifficultyDropdown from"../scripts/components/DifficultyDropdown.min.js";// Functions and variables specific to the tossups page.
 ...
 * Get a random tossup.
 * @returns
 */async function getRandomTossup({alternateSubcategories:a,categories:b,difficulties:c,minYear:d,maxYear:e,powermarkOnly:f,subcategories:g,standardOnly:h}={},i=null){if(i?.percentView)return b=[i.getRandomCategory()],g=[],a=[],await loadRandomTossups({alternateSubcategories:a,categories:b,difficulties:c,maxYear:e,minYear:d,powermarkOnly:f,subcategories:g,standardOnly:h}),randomTossups.pop();0===randomTossups.length&&(await loadRandomTossups({alternateSubcategories:a,categories:b,difficulties:c,maxYear:e,minYear:d,number:20,powermarkOnly:f,subcategories:g,standardOnly:h}));const j=randomTossups.pop();// Begin loading the next batch of questions (asynchronously)
return 0===randomTossups.length&&loadRandomTossups({alternateSubcategories:a,categories:b,difficulties:c,maxYear:e,minYear:d,number:20,powermarkOnly:f,subcategories:g,standardOnly:h}),j}async function next(){if(clearTimeout(timeoutID),currentlyBuzzing=!1,settings.timer&&(timer.stopTimer(),timer.tenthsRemaining=0,timer.updateDisplay()),(await account.getUsername())&&document.getElementById("answer").innerHTML){const a=previous.isCorrect?previous.inPower?previous.powerValue:10:previous.endOfQuestion?0:previous.negValue;questionStats.recordTossup(tossups[questionNumber-1],previous.isCorrect,a,previous.celerity,!1)}document.getElementById("answer").textContent="",document.getElementById("question").textContent="",document.getElementById("toggle-correct").textContent="I was wrong",document.getElementById("toggle-correct").classList.add("d-none");const a=await advanceQuestion();a&&(document.getElementById("buzz").textContent="Buzz",document.getElementById("buzz").disabled=!1,document.getElementById("next").textContent="Skip",document.getElementById("packet-number-info").textContent=query.packetNumbers[0],document.getElementById("packet-length-info").textContent=settings.selectBySetName?tossups.length:"-",document.getElementById("pause").textContent="Pause",document.getElementById("pause").disabled=!1,document.getElementById("question").textContent="",document.getElementById("set-name-info").textContent=query.setName,paused=!1,readQuestion(Date.now()))}/**
