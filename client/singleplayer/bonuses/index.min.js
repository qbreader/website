...
case"1":return document.getElementById("checkbox-1").click();case"2":return document.getElementById("checkbox-2").click();case"3":return document.getElementById("checkbox-3").click();case"4":return document.getElementById("checkbox-4").click()}});let startingDifficulties=[];if(window.localStorage.getItem("singleplayer-bonus-query"))try{const a=JSON.parse(window.localStorage.getItem("singleplayer-bonus-query"));if(a.version!==queryVersion)throw new Error;room.categoryManager.import(a),room.query=a,socket.sendToServer({type:"set-packet-numbers",...a}),socket.sendToServer({type:"set-set-name",...a}),socket.sendToServer({type:"set-year-range",...a}),socket.sendToServer({type:"toggle-select-by-set-name",...a}),socket.sendToServer({type:"toggle-standard-only",...a}),socket.sendToServer({type:"toggle-three-part-bonuses",...a}),startingDifficulties=a.difficulties}catch{window.localStorage.removeItem("singleplayer-bonus-query")}if(window.localStorage.getItem("singleplayer-bonus-settings"))try{const a=JSON.parse(window.localStorage.getItem("singleplayer-bonus-settings"));if(a.version!==settingsVersion)throw new Error;socket.sendToServer({type:"toggle-show-history",...a}),socket.sendToServer({type:"toggle-type-to-answer",...a})}catch{window.localStorage.removeItem("singleplayer-bonus-settings")}ReactDOM.createRoot(document.getElementById("category-modal-root")).render(/*#__PURE__*/React.createElement(CategoryModal,{categoryManager:room.categoryManager,onClose:()=>socket.sendToServer({type:"set-categories",...room.categoryManager.export()})})),ReactDOM.createRoot(document.getElementById("difficulty-dropdown-root")).render(/*#__PURE__*/React.createElement(DifficultyDropdown,{startingDifficulties:startingDifficulties??[],onChange:()=>socket.sendToServer({type:"set-difficulties",difficulties:getDropdownValues("difficulties")})}));