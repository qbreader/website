import questionStats from"../../scripts/auth/question-stats.js";import api from"../../scripts/api/index.js";import audio from"../../audio/index.js";import{arrayToRange,createBonusCard,rangeToArray}from"../../scripts/utilities/index.js";import{getDropdownValues}from"../../scripts/utilities/dropdown-checklist.js";import CategoryModal from"../../scripts/components/CategoryModal.min.js";import DifficultyDropdown from"../../scripts/components/DifficultyDropdown.min.js";import ClientBonusRoom from"../ClientBonusRoom.js";import Player from"../../../quizbowl/Player.js";import Team from"../../../quizbowl/Team.js";let maxPacketNumber=24;const queryVersion="2024-11-01",settingsVersion="2024-11-02",USER_ID="user",TEAM_ID="team",room=new ClientBonusRoom;room.players[USER_ID]=new Player(USER_ID),room.players[USER_ID].teamId=TEAM_ID,room.teams[TEAM_ID]=new Team(TEAM_ID);const socket={send:onmessage,sendToServer:a=>room.message(USER_ID,a)};room.sockets[TEAM_ID]=socket;function onmessage(a){const b=JSON.parse(a);switch(b.type){case"clear-stats":return clearStats(b);case"give-answer":return giveAnswer(b);case"next":case"skip":case"start":return next(b);case"set-categories":return setCategories(b);case"set-difficulties":return setDifficulties(b);case"set-packet-numbers":return setPacketNumbers(b);case"set-set-name":return setSetName(b);case"set-year-range":return setYearRange(b);case"start-answer":return startAnswer(b);case"reveal-leadin":return revealLeadin(b);case"reveal-next-answer":return revealNextAnswer(b);case"reveal-next-part":return revealNextPart(b);case"toggle-select-by-set-name":return toggleSelectBySetName(b);case"toggle-show-history":return toggleShowHistory(b);case"toggle-standard-only":return toggleStandardOnly(b);case"toggle-three-part-bonuses":return toggleThreePartBonuses(b);case"toggle-type-to-answer":return toggleTypeToAnswer(b)}}function clearStats({teamId:a}){updateStatDisplay({0:0,10:0,20:0,30:0})}async function giveAnswer({currentPartNumber:a,directive:b,directedPrompt:c}){if("prompt"===b)return document.getElementById("answer-input-group").classList.remove("d-none"),document.getElementById("answer-input").focus(),void(document.getElementById("answer-input").placeholder=c?`Prompt: "${c}"`:"Prompt");document.getElementById("answer-input").value="",document.getElementById("answer-input").blur(),document.getElementById("answer-input").placeholder="Enter answer",document.getElementById("answer-input-group").classList.add("d-none");"accept"===b?(document.getElementById(`checkbox-${a+1}`).checked=!0,document.getElementById("reveal").disabled=!1,audio.soundEffects&&audio.correct.play()):"reject"===b?(document.getElementById("reveal").disabled=!1,audio.soundEffects&&audio.incorrect.play()):void 0}/**
 ...
 */async function next({type:a,bonus:b,lastPartRevealed:c,oldBonus:d,packetLength:e,pointsPerPart:f,stats:g,teamId:h}){"start"===a&&(document.getElementById("next").disabled=!1,document.getElementById("settings").classList.add("d-none")),c&&(questionStats.recordBonus(d,f),updateStatDisplay(g)),("next"===a||"skip"===a)&&createBonusCard(d),document.getElementById("question").textContent="",document.getElementById("reveal").disabled=!1,document.getElementById("next").textContent="Skip",document.getElementById("set-name-info").textContent=b.set.name,document.getElementById("packet-number-info").textContent=b.packet.number,document.getElementById("packet-length-info").textContent=room.query.selectBySetName?e:"-",document.getElementById("question-number-info").textContent=b.number}/**
