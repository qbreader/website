 ...
 * @param {string} object.setNames - an array of sets to include (e.g. ["2021 ACF Fall"]).
 ...
 */
async function getSets ({ setNames, packetNumbers, categories, subcategories, questionType = 'tossup', reverse = false }) {
  if (!setNames) return [];
  if (!Array.isArray(setNames)) { setNames = [setNames]; }
